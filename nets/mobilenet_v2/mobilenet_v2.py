...
class mobilenetv2(Network):
    ...
    def _image_to_head(self, is_training, reuse=None):
        assert (0 <= cfg.MOBILENET_V2.FIXED_LAYERS <= 19)
        # print('>>>>>>', V2_DEF)
        net_conv = self._image
        # end_points = {}
        with slim.arg_scope(training_scope(is_training=is_training)):
            net_conv, end_points = mobilenet_base(net_conv, conv_defs=V2_DEF, depth_multiplier=self._depth_multiplier,
                                             min_depth=self._min_depth)
        # if cfg.MOBILENET_V2.FIXED_LAYERS > 0:
        #     with slim.arg_scope(training_scope(is_training=False)):
        #         net_conv, end_point = mobilenet(net_conv, starting_layer=0,
        #                                         conv_defs={'defaults': V2_DEF['defaults'],
        #                                                    'spec': V2_DEF['spec'][:cfg.MOBILENET_V2.FIXED_LAYERS]},
        #                                         depth_multiplier=self._depth_multiplier, min_depth=self._min_depth)
        #         end_points.update(end_point)
        # if cfg.MOBILENET_V2.FIXED_LAYERS < 19:
        #     with slim.arg_scope(training_scope(is_training=is_training)):
        #         net_conv, end_point = mobilenet(net_conv, starting_layer=cfg.MOBILENET_V2.FIXED_LAYERS,
        #                                         conv_defs={'defaults': V2_DEF['defaults'],
        #                                                    'spec': V2_DEF['spec'][cfg.MOBILENET_V2.FIXED_LAYERS:]},
        #                                         depth_multiplier=self._depth_multiplier, min_depth=self._min_depth)
        #         end_points.update(end_point)
